...
- **收藏夹**：按照 Raindrop Collection 与子 Collection 的原始层级展示；
- **标签**：自动聚合 Raindrop Tags，每个标签作为独立分类，按使用数量排序；同一网站可以出现在多个标签分类中，无标签项目统一归入“无标签”。

搜索会同时匹配网站标题、域名、简介与标签，并在两种视图中保持一致。浏览器会记住最近一次选择的分类视图。
